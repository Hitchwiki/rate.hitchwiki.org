...
require_once("{$IP}/extensions/Rate/functions.inc.php");

$imgpath = '/wiki/extensions/Rate/widgets/mediawiki/img';

$wgExtensionFunctions[] = "wfcountryrating";

$wgExtensionCredits['parserhook'][] = array(
    'name' => 'Country Ratings',
    'author' => '[http://www.philippgruber.de/ Philipp Gruber], [http://www.mikaelkorpela.fi/ Mikael Korpela]',
...
function countryrating($input, $argv) {
    ...
    if (!isset($argv['country']) || strlen($argv['country']) != 2) {
        return '<span class="label label-danger">No valid country specified</span>';
    }
...
<span id='rating_$country' class='rating'>
    ...
    function hwr_rateCountry(country, value) {
        ...
        http_request.open('GET', mw.config.get('wgExtensionAssetsPath') + '/Rate/rate.php?country=' + country + '&rating=' + value, true);
        http_request.onreadystatechange = function () {
            if (http_request.readyState == 4 && http_request.status == 200) {
                //console.log('->hwr_rateCountry: http_request.onreadystatechange');
                //console.log('->hwr_rateCountry: ' + http_request.response);
    ...
    <img src='$imgpath/hitch".round($rating['rating']).".png' /> "._($ratingName[round($rating['rating'])])."
    <a onclick='document.getElementById(\"hwr_rateselect_$country\").style.display = \"block\"' class=\"btn btn-default btn-mini btn-xs\">"._("Rate!")."</a>
    <span id='hwr_rateselect_$country' class='hwr_ratepopup'>
        <div class='hwr_current_rating'>
            ...
            <br/><br/>
            <b class='hwr_your_rate'>"._("Your rating").":</b><br />
